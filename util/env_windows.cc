...
public:
	...
	WindowsMmapReadableFile(
		std::string filename, char* mmap_base, size_t length, Limiter* mmap_limiter)
...
public:
	...
	Status NewSequentialFile(const std::string& filename, SequentialFile** result) override {
		*result = nullptr;
		...
		ScopedHandle handle = ::CreateFileA(filename.c_str(), desired_access, share_mode,
	...
	Status GetChildren(
		const std::string& directory_path, std::vector<std::string>* result) override {
	...
	Status LockFile(const std::string& filename, FileLock** lock) override {
		*lock = nullptr;
		Status result;
		ScopedHandle handle =
			::CreateFileA(filename.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
...
void WindowsEnv::Schedule(
	void (*background_work_function)(void* background_work_arg), void* background_work_arg) {
