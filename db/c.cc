...
uint8_t leveldb_iter_valid(const leveldb_iterator_t* iter) { return iter->rep->Valid(); }
...
void leveldb_writebatch_put(
	leveldb_writebatch_t* b, const char* key, size_t klen, const char* val, size_t vlen) {
...
void leveldb_writebatch_iterate(const leveldb_writebatch_t* b, void* state,
	...
	public:
		...
		void Delete(const Slice& key) override { (*deleted_)(state_, key.data(), key.size()); }
	};
	H handler;
...
void leveldb_writeoptions_set_sync(leveldb_writeoptions_t* opt, uint8_t v) { opt->rep.sync = v; }
