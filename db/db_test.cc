...
static void MTThreadBody(void* arg) {
	...
	while (!t->state->stop.load(std::memory_order_acquire)) {
		...
		if (rnd.OneIn(2)) {
			...
			std::snprintf(
				valbuf, sizeof(valbuf), "%d.%d.%-1000d", key, id, static_cast<int>(counter));
...
static bool CompareIterators(
	int step, DB* model, DB* db, const Snapshot* model_snap, const Snapshot* db_snap) {
	ReadOptions options;
