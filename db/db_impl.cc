...
void DBImpl::RemoveObsoleteFiles() {
	...
	for (std::string& filename : filenames) {
		if (ParseFileName(filename, &number, &type)) {
			...
			case kLogFile:
				keep =
					((number >= versions_->LogNumber()) || (number == versions_->PrevLogNumber()));
				break;
...
Status DBImpl::Recover(VersionEdit* edit, bool* save_manifest) {
	...
	if (!expected.empty()) {
		char buf[50];
		std::snprintf(
			buf, sizeof(buf), "%d missing files; e.g.", static_cast<int>(expected.size()));
...
Iterator* DBImpl::NewInternalIterator(
	const ReadOptions& options, SequenceNumber* latest_snapshot, uint32_t* seed) {
	mutex_.Lock();
...
bool DBImpl::GetProperty(const Slice& property, std::string* value) {
	...
	if (in.starts_with("num-files-at-level")) {
		...
		if (!ok || level >= config::kNumLevels) {
			...
			char buf[100];
			std::snprintf(
				buf, sizeof(buf), "%d", versions_->NumLevelFiles(static_cast<int>(level)));
			*value = buf;
			return true;
		}
	} else if (in == "stats") {
